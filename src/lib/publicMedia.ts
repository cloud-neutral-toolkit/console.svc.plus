...
async function getCachedData<T>(
  ...
  try {
    // 1. Check if cache exists and source file exists
    ...
    if (cacheStats && sourceStats) {
      // 2. For development, always refresh cache to get the latest images
      // In production, you might want to use a time-based invalidation
      const isDev = process.env.NODE_ENV === "development";
      const cacheIsOld = cacheStats.mtimeMs < sourceStats.mtimeMs;

      if (!isDev && !cacheIsOld) {
        // In production and cache is fresh, use cached data
