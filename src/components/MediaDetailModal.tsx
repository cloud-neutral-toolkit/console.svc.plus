...
export default function MediaDetailModal({
    ...
    return (
      ...
      `}</style>
                ...
                >
                    ...
                    <div className="absolute bottom-6 left-6 flex gap-3 z-10">
                        <button
                            ...
                            title={layout === 'side' ? "Switch to Stacked View" : "Switch to Free View"}
                        >
                            {layout === 'side' ? <AlignVerticalJustifyCenter size={20} /> : <AlignLeft size={20} />}
                        </button>
                        <button
          ...
          ${layout === 'side'
                            ? (isTextCollapsed ? 'w-0 opacity-0' : 'w-1/3 opacity-100 border-l border-slate-100')
                            : (isTextCollapsed ? 'h-0 opacity-0' : 'h-1/3 opacity-100 border-t border-slate-100')
                        }`}
                >
                    <div className="h-full overflow-y-auto px-8 py-10">
                        <div className="mb-8 border-b border-slate-100 pb-6">
                            <h2 className="text-3xl font-bold text-slate-900 leading-tight">{item.title}</h2>
                            <div className="mt-4 flex flex-wrap gap-4 text-sm text-slate-500">
                                {item.location && (
                                    <div className="flex items-center gap-1">
                                        <span>📍</span>
                                        <span>{Array.isArray(item.location) ? item.location.join(', ') : item.location}</span>
                                    </div>
                                )}
                                {item.date && (
                                    <div className="flex items-center gap-1">
